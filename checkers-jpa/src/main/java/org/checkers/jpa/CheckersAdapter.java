...
public class CheckersAdapter implements CheckersGame {
	...
	public CheckersAdapter(CheckersDAO dao, Game game) {
		...
		for (Turn turn : game.getTurns()) {
			this.coreGame.movePiece(turn.getColour(), turn.getColumnSelected(), turn.getRowSelected(),
					turn.getColumnTargeted(), turn.getRowTargeted());
		}
	}
	...
	public void movePiece(PieceColour colour, int columnSelected, int rowSelected, int columnTargeted, int rowTargeted)
		...
		try {
			...
			this.game.getTurns()
					.add(new Turn(this.game, colour, columnSelected, rowSelected, columnTargeted, rowTargeted));
			switchTurn();
	...
	public void switchTurn() {
		game.setCurrentTurn(game.getCurrentTurn() == PieceColour.BLACK ? PieceColour.WHITE : PieceColour.BLACK);
	}
